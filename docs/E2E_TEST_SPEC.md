...
- サーバーの `_audio_listeners` にゴミが残っていない（`/api/status` の `listening_clients` が 0 または 1）
...
### 2. `release_talk()` の無条件呼び出し

`audio_disconnect()` で、talk していないクライアントの切断時にも `release_talk()` が呼ばれる。
`max(0, ...)` で保護されているが、`_talking_sid` のチェックにより正確なクリーンアップが行われる。

### 3. `async_mode="threading"` での同期ブロック
