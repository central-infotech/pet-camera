...
| カメラデバイス指定 | 環境変数 `PET_CAMERA_INDEX` でカメラデバイス番号を明示指定可能。未設定時は IR カメラを自動スキップして RGB カメラを選択 |
...
起動時に `enumerate_cameras()` で接続中の全カメラデバイス（最大 10 台）をプローブし、IR カメラ（赤外線カメラ）を自動スキップする。
...
これにより、Windows Hello 顔認証用 IR カメラを搭載した外付け USB カメラ（例: ELECOM UCAM-CF20FBBK）でも、RGB カメラが自動選択される。
