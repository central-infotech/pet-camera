...
- **PWA スタンドアロン対応**: `height: 100dvh` + `overflow: hidden` でビューポートに完全収まるレイアウト。映像エリアは `min-height: 0` で flex 縮小を許可し、ヘッダー・コントロール・フッターを配置した残り領域にフィットさせる。`viewport-fit=cover` + `env(safe-area-inset-bottom)` でノッチ・ジェスチャーナビゲーション領域に対応
