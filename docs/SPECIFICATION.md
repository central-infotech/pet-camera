...
└──────────────────────────────┼────────────────────────────┘
          ...
          Phase 1 映像: PC → クライアント（WebRTC H.264 単方向）
...
| Web サーバー | Flask (Python) | 軽量で API サーバーとして実績が豊富 |
...
| 映像配信 | WebRTC (H.264, aiortc) | H.264 圧縮により低帯域で高品質な映像配信。Tailscale VPN 内では STUN/TURN 不要 |
...
| # | 機能 | 説明 |
|---|------|------|
| F-01 | ライブ映像配信 | WebRTC (H.264) でリアルタイム映像を配信 |
...
| F-05 | カメラ設定調整 | カメラデバイス選択・解像度・フレームレート・明るさ・コントラストを UI から変更可能。IR カメラの自動スキップ対応 |
...
| F-21 | 多重起動防止 | BroadcastChannel API を使い、同一ブラウザ/PWA で複数タブ・インスタンスが同時に開かれた場合、古いインスタンスの WebRTC・音声接続を自動的に切断する。通信量の意図しない倍増を防止 |
...
カメラデバイスドロップダウンには接続中の全カメラが表示される。IR カメラ（Windows Hello 顔認証用赤外線カメラ等）には `[IR]` ラベルが付与される。起動時にはIR カメラを自動スキップし、RGB カメラを優先選択する。
...
│                          │
└──────────────────────────┘
  利用規約・プライバシーポリシー     ← テキストリンク（カード下部）
```

- **利用規約・プライバシーポリシー**: ログインカード下部にテキストリンクを表示
  - タップするとモーダルオーバーレイ（縦スクロール可）で全文を表示
  - 閉じるボタン（×）またはオーバーレイ背景タップで閉じる
  - 内容: 本サービスについて、免責事項、データの取り扱い、禁止事項、お問い合わせ
...
│                                                   │
│                 信                                 │  ← 画面中央に縦書き表示
│                 号                                 │    （writing-mode: vertical-rl）
│                 な                                 │    （#888、2rem）
│                 し                                 │
│                                                   │
│                                                   │
│                                                   │
│   接続待機中... / 映像が途切れました               │  ← ステータス表示（下部、小さく）
└─────────────────────────────────────────────────┘
```

- **背景色**: 黒（ペットの気を散らさないよう最小限のUI）
- **映像表示**: 画面全体に `width:100%; height:100%` + `object-fit:contain` で最大表示（アスペクト比維持、余白は黒）。映像未受信時は `<img>` を非表示にし、黒背景のみ
- **信号なし表示**: 映像未受信時に画面中央に「信号なし」を縦書き（`writing-mode: vertical-rl`）で大きめ（2rem、グレー #888）表示。映像受信開始で自動的にフェードアウト
...
│            └──────────────────────┘               │
├─────────────────────────────────────────────────┤
│  [🔊 聞く] [🎤 話す] [📹 顔を見せる] 音量 [━━━]  │  ← 操作ボタン（横スクロール）
├─────────────────────────────────────────────────┤
│  送信中: 640x480 / 10fps / 接続PC: 1台            │  ← ステータス行（送信中のみ表示）
├─────────────────────────────────────────────────┤
│  [📷 スマホに画像保存] [📁 PCに画像保存] [⚙ 設定] │  ← 保存・設定ボタン
└─────────────────────────────────────────────────┘
```

| ボタン / UI | 動作 | 説明 |
|------------|------|------|
| 📹 顔を見せる | トグル（ON/OFF） | ON にするとフロントカメラ映像の送信を開始。映像のみで音声には影響しない（聞く・話すボタンと完全に独立） |
| PiP ワイプ | 「顔を見せる」連動 | ON 時にメイン映像エリアの左上にフロントカメラのプレビュー（120px幅）を小窓表示。OFF 時は非表示。映像は左右反転（鏡像） |
| ステータス行 | 自動表示/非表示 | 送信中・再接続中など表示すべき情報がある場合のみ独立した行として表示。空の場合は `:empty` で非表示（行ごと消える） |

### 5.7 レスポンシブ対応

- **デスクトップ（Phase 1 ビューワー）**: 映像を最大表示
- **デスクトップ（Phase 2 飼い主表示）**: 全画面黒背景に映像のみ表示
- **モバイル縦画面（主要ユースケース）**: 映像を画面幅にフィット、音声コントロール・操作ボタンは下部に固定。操作パネルはトグルボタン（▲/▼）で表示/非表示を切り替え可能。2段構成の操作ボタンは横スクロール対応
- **モバイル横画面**: `@media (orientation: landscape) and (max-height: 500px)` でスマホの横画面を検出し、専用レイアウトに切り替え。`<main>` に `display: contents` を適用し、子要素を body の flex item として横配列にする
  - **左サイドバー**（~70px、常時表示）: ロゴ、LIVE バッジ、経過時間、画面更新ボタン（アイコンのみ）を縦に配置
  - **中央**（flex:1）: 映像エリアが縦幅いっぱいに最大表示
  - **右トグル**（~22px）: ◀/▶ ボタンで右パネルを開閉。`writing-mode: vertical-rl` で縦方向に配置
  - **右パネル**（~140px、トグルで開閉）: 聞く、話す、顔を見せる、音量、保存、設定ボタンを縦に積み、下部にステータス情報を表示（フッターは非表示にし、右パネル内の `.landscape-status` で代替。音声ステータスはスラッシュ区切りではなく改行で表示）
- **画面回転対応**: `manifest.json` から `orientation` フィールドを削除（最も許容的）し、JavaScript で `screen.orientation.unlock()` を呼び出すことで、Android PWA でも自由に画面回転可能
...
|-----------------|------------|--------------|------------------|
| Android | Chrome | カスタムインストールプロンプト（`beforeinstallprompt`） | `display: standalone` |
| iOS | Safari | `apple-mobile-web-app-capable` メタタグ | ステータスバー: `black-translucent` |

#### インストールプロンプト（カスタム UI）

ブラウザの `beforeinstallprompt` イベントを利用し、独自のインストールダイアログを表示する。

- **表示条件**: ブラウザが PWA インストール可能と判定し、かつ以下をすべて満たす場合
  - スタンドアロンモードで起動していない（`display-mode: standalone` 非該当）
  - ユーザーが過去に「ブラウザで表示する」を選択していない（`localStorage` で記憶）
- **選択肢**:
  - 「アプリをインストールする」→ ネイティブインストールフロー起動
  - 「このままブラウザで表示する」→ ダイアログ閉じ、`localStorage` にフラグ保存（再表示しない）
- **既インストール時**: `beforeinstallprompt` イベントが発火しないため、プロンプトは表示されない
- **iOS**: `beforeinstallprompt` 非対応のため、Safari の「ホーム画面に追加」に依存

#### 構成ファイル

- **manifest.json**: アプリ名・アイコン・表示モード・テーマカラーを定義
- **sw.js（Service Worker）**: アプリシェル（HTML・CSS・JS・アイコン）をキャッシュ。ストリーミングデータ（WebSocket・API）はキャッシュしない
...
| GET | `/api/cameras` | 必要 | 接続中のカメラデバイス一覧を取得（IR 判定付き） |
| PATCH | `/api/cameras/current` | 必要 | 使用するカメラデバイスを切り替え |
...
> **注意**: 「顔を見せる」「聞く」「話す」の3ボタンは完全に独立して動作する。同一スマホ上ではこれら3機能を自由に組み合わせて利用可能。ハウリング防止の自動制御は行わない。
...
接続中のカメラデバイス一覧と現在使用中のインデックスを返す。

```json
{
  "cameras": [
    { "index": 0, "name": "Camera 0 (DSHOW)", "width": 640, "height": 480, "is_ir": true },
    { "index": 1, "name": "Camera 1 (DSHOW)", "width": 1280, "height": 720, "is_ir": false }
  ],
  "current_index": 1
}
```

`is_ir` はテストフレームのピクセル分析による IR カメラ（赤外線カメラ）判定。非常に暗い、またはグレースケールに近いフレームを出力するデバイスを IR と判定する。

#### PATCH `/api/cameras/current`
...
切り替え時はカメラスレッドを停止・再起動し、WebRTC ソーストラックもリセットする。
...
| カメラデバイス指定 | 環境変数 `PET_CAMERA_INDEX` でカメラデバイス番号を明示指定可能。未設定時は DirectShow フレンドリー名から `ELECOM 2MP Webcam` を優先、`Integrated Camera`/`Virtual Camera`/`Face Authentication` をブロックリストで除外、残った非 IR デバイスを選択 |
...
```
2026-02-19T14:30:52+09:00 | 100.100.1.51 | POST /api/webrtc/offer | 200
...
| R-06 | カメラデバイス障害 | WebRTC 映像が停止 | カメラスレッドの自動リトライ + 自動リカバリ（別カメラへの自動切り替え） |
...
| 復帰時のローディング表示 | `app.js`, `index.html`, `style.css` | 復帰時は WebRTC 接続状態に関わらず常にスピナーアニメーション付き「読み込み中...」オーバーレイを表示。iOS がバックグラウンドで video 要素を一時停止するため `play()` を明示的に呼び出す。WebRTC 接続中の場合は `requestVideoFrameCallback`（非対応ブラウザでは `currentTime` ポーリング）で実際の映像フレーム描画を検知してから非表示にする（5秒タイムアウト）。再接続が必要な場合は `onConnected` で非表示。全ケース共通で15秒のグローバル安全タイムアウト付き（復旧失敗時も画面が永久にフリーズしない） |
...
カメラスレッド（`camera.py`）はリトライ機構（5秒間隔で再試行）と自動リカバリ機構を持つ。

| 対策 | 説明 |
|------|------|
| 自動リトライ | カメラオープン失敗・フレーム取得失敗時に 5秒（オープン失敗）/1秒（フレーム取得失敗）間隔でリトライ |
| 自動リカバリ | 連続失敗が閾値（3回）に達した場合、`enumerate_cameras()` で別のカメラを自動スキャンし、IR 以外の利用可能なカメラに自動切り替え。切り替え時は WebRTC ソーストラックも自動リセットする |
| 復旧通知 | カメラの自動リカバリ成功時にログレベル INFO で記録（切り替え元・切り替え先のインデックスを出力） |

**自動リカバリの動作フロー:**
1. カメラのオープンまたはフレーム取得が 3 回連続で失敗
2. `enumerate_cameras()` で接続中の全カメラをスキャン
3. 現在のインデックス以外で、IR でないカメラを順に試行
4. オープンに成功したカメラに自動切り替え
5. コールバック経由で WebRTC ソーストラック（`_source_track`）をリセットし、映像配信を復旧
6. 利用可能なカメラが見つからない場合は、従来のリトライループを継続

これにより、USB カメラの抜き差しや内蔵カメラから外付けカメラへの変更時でも、サーバー再起動なしで自動的に映像が復旧する。

#### カメラデバイス自動検出

起動時に `enumerate_cameras()` で接続中の全カメラデバイス（最大 10 台）をプローブし、各デバイスの **DirectShow フレンドリー名**（`pygrabber` 経由）と IR 判定結果を取得する。`find_best_camera_index()` は以下の優先順で選択する:

1. **優先デバイス名マッチ** — 名前に `ELECOM 2MP Webcam` を含むデバイス。本デプロイの正規カメラを名前ベースで明示固定し、列挙順の揺れや故障した内蔵カメラを掴むリスクを排除する
2. **ブロックリスト除外 かつ 非 IR** — 名前に `Virtual Camera` / `Face Authentication` / `Integrated Camera` を含むデバイスは選択しない（Windows Hello 仮想カメラ・顔認証用 IR・Lenovo 内蔵カメラの不調個体を除外）
3. **非 IR デバイス**
4. **先頭デバイス**（最終フォールバック）
...
1. テストフレームを 1 枚取得
2. 平均輝度が 5 未満 → IR と判定
3. 平均輝度が 30 未満かつ R/G/B チャンネル差が 3 未満（グレースケール） → IR と判定

これにより、Windows Hello 顔認証用 IR カメラを搭載した外付け USB カメラ（例: ELECOM UCAM-CF20FBBK）でも、RGB カメラが自動選択される。`Integrated Camera` がドライバ Error 状態で黒フレームを返し続ける症状の再発防止として、優先名マッチを追加している。

環境変数 `PET_CAMERA_INDEX` が設定されている場合は自動検出をスキップし、指定されたインデックスを使用する。UI の設定パネルからランタイムでカメラデバイスを切り替えることも可能。
...
| 障害シナリオ | 症状 | 復旧方法 |
|------------|------|---------|
| Tailscale VPN 一時切断 | 全 WebSocket が切断。WebRTC ストリームが停止 | Tailscale 自体が自動再接続する。クライアント側 Socket.IO 自動再接続（R-01）+ WebRTC 自動再接続で復旧 |
...
2. フォアグラウンド復帰時:
   ...
   - WebRTC 切断の場合: 再接続を試行 → `onConnected` でオーバーレイ非表示
...
| **P2（実装済）** | カメラ自動リカバリ（R-06） | 連続失敗時に別カメラへ自動切り替え。サーバー再起動不要 |
...
> Flask-SocketIO は `ssl_context` を指定すると HTTPS + WSS を同一ポートで同時に提供する。映像（WebRTC シグナリング）・音声（WSS）・API（HTTPS）のすべてが `:5555` に統一される。
...
| カメラデバイス | `PET_CAMERA_INDEX` 未設定時は自動検出（IR スキップ） | 同左。明示指定も可 |
...
| 逆方向 HTTP ストリーム | スマホ側に HTTP サーバーが必要になり、ブラウザでは実現困難 |
...
Step 3: スマホ側 — カメラ送信機能追加 (static/js/app.js)
  - 「顔を見せる」トグルボタンの追加
  - getUserMedia でフロントカメラ（映像のみ）取得
  - <video> → <canvas> → toBlob で JPEG エンコード
  - setInterval で定期的に video_frame イベントを送信
  - 音声機能とは完全に独立（聞く・話すは別途ボタン操作が必要）

Step 4: スタイル調整 (static/css/style.css)
...
- WebRTC (H.264) による映像配信で帯域効率とリアルタイム性を両立
- Phase 1 の音声はプッシュ・トゥ・トーク方式とし、常時双方向通話はサポートしない
- Phase 2 の「顔を見せる」「聞く」「話す」は完全に独立したトグル。同一スマホ上ではこれら3機能を自由に組み合わせて利用可能（ハウリング防止の自動制御は行わない）
