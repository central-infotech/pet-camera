...
- 「スナップショット」→「📷 スマホに画像保存」: スマホ端末にダウンロード
- 「保存」→「📁 PCに画像保存」: サーバー（PC）の `snapshots/` ディレクトリに JPEG 保存
- 設定ボタンを独立行に分離（`save-controls` + `settings-controls`）
- 1行増加分は `.controls` の padding を縮小（`0.35rem`）して吸収し、スクロールなしで1画面に収める
